...
from api_test_ez.core.report import BRReporter, DryRun, HtmlReporter
from api_test_ez.project import Project

EZ_SUPPORT_ACTION = ["run", "dry-run"]
...
class EzCommand:
    ...
    def parse_ez_parent():
        ...
        parser.add_argument(
            "action",
            choices=["run"],
            help="Command action." "For example: `ez run` <test_cases_path>",
        )
        # run cases
        parser.add_argument(
            "cases_path",
    ...
    def parse_ez_args():
        ...
        parser.add_argument(
            "action",
            choices=["run", "dry-run"],
            help="Command action." "For example: `ez run`",
        )
        # run cases
        parser.add_argument(
            "cases_path",
        ...
        parser.add_argument(
            "-t", "--tester", dest="tester", default="Jenkins", help="Tester name."
        )
        ...
        # report file
        parser.add_argument(
            "-rf", "--report-file", dest="report_file", help="Report file path."
        )
        return parser
    ...
    def process_args(self, argv):
        ...
        if len(argv) == 1:
            print(
                f"EZ COMMAND ERROR: expect a <case-file-path> after the action-word `{argv[0]}`. See: \n"
            )
            ez_parser.print_help()
            return
        ...
        if argv[1].startswith("-"):
            print(
                f"EZ COMMAND ERROR: expect a <case-file-path> after the action-word `{argv[0]}`, "
        ...
        project = Project(
            ez_file_path=(
                cases_path if os.path.isdir(cases_path) else os.path.dirname(cases_path)
            )
        )
        ...
        if args.report_file:
            report_dir = (
                os.path.dirname(args.report_file)
        ...
        if args.config_file:
            if os.path.exists(args.config_file):
                print(
                    f"EZ COMMAND ERROR: can not find ez-config-file `{args.config_file}`."
                )
                ez_parser.print_help()
            else:
        ...
        if args.config:
            for cfg in args.config:
                if "=" not in cfg:
                    print(
                        "EZ COMMAND ERROR: config format error, For example: `-cfg host=127.0.0.1`. See: \n"
                    )
                    ez_parser.print_help()
                    return
        ...
        if args.action == "run":
            if args.framework == "pytest":
                try:
                    ...
                    print(
                        "EZ COMMAND ERROR: pytest is required when `--framework pytest`."
                    )
                    return
                pytest_args = [args.cases_path]
            ...
            if project.report.report_dir:
                if args.report_style == "br":
                    ...
                    print(
                        f"EZ COMMAND WARNING: `{args.report_style}` is not supported `report_style`, "
                        f"run tests as dry-run. See: \n"
                    )
                    ez_parser.print_help()
            else:
                print(
                    f"EZ COMMAND WARNING: `report_dir` does not set, run tests as dry-run.\n"
                )
                # ez_parser.print_help()
